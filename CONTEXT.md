...
- **Frontmatter readout**: the literal `key: "value"` block rendered under a project page's h1 — title, subtitle, tags, pageNumber always shown, `context` shown only when authored; makes the page's own frontmatter part of the display instead of hiding it. Project pages only, not blog; see `docs/PRD-p204-design-system-revamp.md` §4.5.
- **Interview quote / Callout**: the two blockquote registers in prose content. An interview quote (cyan border + text, grey attribution) is detected automatically when the blockquote's last line starts with an em dash (`—`); everything else — e.g. "Key learning:" — renders as a callout (no border, plain white text). No separate authoring syntax; see `docs/PRD-p204-design-system-revamp.md` §4.4.
