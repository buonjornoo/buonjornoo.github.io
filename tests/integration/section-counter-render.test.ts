 ...
 * "k/N" in the page-title slot; index pages don't get the slot at all.
 * Long-form detection comes from the layout used, not a heading-count
 * heuristic (issues/13.md: "not a heuristic").
 */
describe('section counter — page-title slot (issues/13)', () => {
  ...
  it('never appears on index pages — Home, Projects, Blog, Contact', () => {
    for (const page of ['index.html', join('projects', 'index.html'), join('blog', 'index.html'), join('contact', 'index.html')]) {
      ...
      expect(html).not.toContain('ceefax-page-title');
    }
  });
});
